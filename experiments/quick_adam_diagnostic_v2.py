...
========================================
Day 3 Stage 1 (v2): probe 4x4 NQS landscape ruggedness via Adam baseline.

v1 (quick_adam_diagnostic.py) was invalidated by RNG bug: J1J2Problem._gen
hardcoded to seed=0 regardless of torch.manual_seed/np.random.seed; 3 seeds
produced byte-identical trajectories. Fix in commit 18716b3 added
J1J2Problem.set_seed(seed). v2 calls it.

5 seeds x Adam M=1 x n_steps=2000 x n_samples=2000 x n_final=4 (mean)

Decision rule (same as v1):
  sigma_Adam / |E_0| < 0.5%: WARNING (landscape too smooth)
  sigma_Adam / |E_0| in [0.5%, 1%]: MARGINAL
  sigma_Adam / |E_0| in [1%, 2%]: GO
  sigma_Adam / |E_0| >= 2%: STRONG GO
  + cluster signal (gap_ratio) modifies verdict (see v1 outline)

Run:
    TCBM_DEVICE=cuda:3 python -u experiments/quick_adam_diagnostic_v2.py 2>&1 \
      | tee logs/adam_v2_$(date +%Y%m%d_%H%M).log

Expected wall: ~2.5h (single GPU sequential, 5 seeds x ~32 min).
"""
import sys
import os
import json
...
OUTPUT_PATH = Path('results/quick_adam_diagnostic_v2.json')
...
def run_one_seed(seed):
    ...
    problem.set_seed(seed)  # ← THE FIX (commit 18716b3)
...
def main():
    print(f"Quick Adam diagnostic v2 (RNG-fixed): {len(SEEDS)} seeds x {N_STEPS_PER_SEED} steps x Adam M=1")
    ...
    print(f"RNG fix: J1J2Problem.set_seed (commit 18716b3)")
    ...
    print(f"\n{'='*70}")
    print(f"DIAGNOSTIC v2 COMPLETE in {elapsed/60:.1f} min ({elapsed/3600:.2f}h)")
    print(f"{'='*70}")
