...
print('handlers installed, sleeping...', flush=True)
time.sleep(60)
""")

    proc = subprocess.Popen([sys.executable, str(test_script)],
                            stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    time.sleep(2)  # let handlers install
    ...
    try:
        ...
        proc.kill()
        assert False, "subprocess did not exit within 5s of SIGTERM"

    # Verify dump within 2s of SIGTERM (we waited 5s total)
    state_files = list(tmp_path.glob('sigterm_subprocess_signal_handler_*'))
    assert len(state_files) >= 1, f"expected dump file from signal handler, got: {list(tmp_path.iterdir())}"
